...
static const char *classPathName = "com/mct/coreservices/OBDSerial";

JavaVM *gJavaVM;
jclass gJavaClass;

static jboolean
openSerial(JNIEnv *env, jobject thiz,jstring portname)
{
    LOGI("open serial port start\n");
    const char *serialport = env->GetStringUTFChars(portname, NULL);
...
static jboolean
closeSerial(JNIEnv *env, jobject thiz)
{
...
static jboolean
writeData(JNIEnv *env, jobject thiz, jint length, jbyteArray data)
{
    LOGI("writeData");
    jbyte sendData[length];
    env->GetByteArrayRegion(data, 0, length, sendData);
    return Serialport_Write((uint8*) sendData, length);
}
...
static JNINativeMethod methods[] = {
        { "openSerial", "(Ljava/lang/String;)Z",(void *) openSerial },
        { "closeSerial", "()Z",(void *) closeSerial },
        { "writeData", "(I[B)Z",(void *) writeData }, };
