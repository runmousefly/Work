...
public final class MctDeviceManager
{
	private static final String TAG = "MctDeviceManager";
	...
	private static MctDeviceManager mManager = new MctDeviceManager();
	private static IDeviceManager sService = null;
	
	

	public static MctDeviceManager getInstance()
	{
		return (getService() == null) ? null : mManager;
	}

	private static synchronized IDeviceManager getService()
	{
		...
		sService = IDeviceManager.Stub.asInterface(b);
		return sService;
	}
	...
	{
		IDeviceManager service = getService();
		try {
	 ...
	 */
	public boolean registerHandler(int[] propIds, DeviceInterfaceDataHandler handler)
	{
		IDeviceManager service = getService();
		try 
		{
	 ...
	 *         might be few changes going forward.
	 */
	public boolean removeHandler(int[] propIds, DeviceInterfaceDataHandler handler)
	{
		IDeviceManager service = getService();
		try 
		{
	...
	private class ServiceRespHandler extends Handler
	{
		...
		public void handleMessage(Message msg)
		{
			...
			if(data != null && mClientCallbackVector!= null)
			{
				...
				{
					((DeviceInterfaceDataHandler) handler).onDataUpdate(propId,value);
				}
			}
		}
	}
	...
	private void notifyFatalError()
	{
		if (mClientCallbackVector != null)
		{
			for (Object handler : mClientCallbackVector)
			{
				((DeviceInterfaceDataHandler) handler).onError(true);
			}
		}
	}
};
